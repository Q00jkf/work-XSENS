...
// Use built-in Serial1 and Serial2 from Arduino core
// Only define additional serial ports if needed
...
// Additional serial ports - commented out for now to avoid conflicts
// Can be enabled later if more hardware serial ports are needed
// Uart Serial3 (&sercom1, 13, 8, SERCOM_RX_PAD_1, UART_TX_PAD_2);
// Uart Serial4 (&sercom3, 10, 9, SERCOM_RX_PAD_3, UART_TX_PAD_2);

// SERCOM handlers for additional serial ports - commented out
// #if !defined(SERCOM1_Handler)
// void SERCOM1_Handler() {Serial3.IrqHandler();}
// #endif

// #if !defined(SERCOM3_Handler)  
// void SERCOM3_Handler() {Serial4.IrqHandler();}
// #endif

void myUART_init(void)
{
    Serial.begin(115200);  // debug and NMEA
    Serial1.begin(230400); // output (PX4) - 恢復原始波特率
    Serial2.begin(115200); // Xsens
    // Serial3.begin(115200); // NMEA_OUT_Serial - not used in this version
    // Serial4.begin(115200); // NMEA_IN_Serial - not used in this version

    // Configure pin peripherals for Serial2 (Xsens) - 修正引腳配置
    pinPeripheral(25, PIO_SERCOM);  // PA15 - Serial2 RX 
    pinPeripheral(24, PIO_SERCOM);  // PA14 - Serial2 TX

    // Additional pin configurations if Serial3/Serial4 are needed later
