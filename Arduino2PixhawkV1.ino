...
// 🔧 新增：檢查點系統和封包監測 (根據PROJECT OVERVIEW)
bool packet_monitoring_enabled = true;  // 封包監測啟用標記
bool ide_connected = false;             // IDE Serial連線狀態

// 🔧 檢查點輸出函數：將關鍵資料流以HEX格式輸出至IDE Serial Monitor
void outputPacketCheckpoint(const char* packet_type, const uint8_t* data, uint16_t length) {
  if (packet_monitoring_enabled && Serial && ide_connected) {
    Serial.print("[");
...
// 🔧 IDE連線檢測函數
void checkIDEConnection() {
  static unsigned long last_check = 0;
  if (millis() - last_check > 5000) { // 每5秒檢查一次
    ide_connected = Serial ? true : false;
    last_check = millis();
  }
}
...
void setup() {
  ...
  // 🔧 根據PROJECT OVERVIEW：檢查IDE連線狀態
  checkIDEConnection();
  
  // 🔧 根據PROJECT OVERVIEW：開機預設進入CONFIG模式
  current_output_mode = OUT_MODE_CONFIG;
  Serial.println("🏁 [CHECKPOINT] System started in CONFIG mode");
  
  Serial.println("All UARTs initialized - Serial1 (Pixhawk): 230400, Serial2 (Xsens): 115200");
  
  checkPX4CON();
  
  // 🔧 CONFIG模式初始化完成後，再切換到MAVLink模式
  Serial.println("🔧 [CHECKPOINT] CONFIG mode initialization completed");
  ...
  Serial.println("🚀 [CHECKPOINT] Set to MAVLink ODOMETRY mode");
...
void loop() { 
  // 🔧 檢查點：檢渫IDE連線狀態(根據PROJECT OVERVIEW)
  checkIDEConnection();
  ...
  // 🔧 增強Serial(USB)雙向通信：IDE對話介面(根據PROJECT OVERVIEW)
  if (Serial.available()){ 
    // 🔧 記錄IDE命令輸入檢查點
    Serial.println("🔧 [CHECKPOINT] IDE command received");
  ...
  // 🔧 優化Serial1輸入處理：減少不必要的雙向通信(根據PROJECT OVERVIEW)
  if (PIXHAWK_SERIAL.available() && enable_input){
    // 🔧 檢查點：Pixhawk數據接收
    Serial.println("🔧 [CHECKPOINT] Pixhawk data received on Serial1");
    ...
    if (current_output_mode == OUT_MODE_XBUS){
      ...
      if (xsens.getDataStatus() == DATA_OK) {
        ...
        if (current_output_mode == OUT_MODE_ML_ODOM && current_Xsens_mode == MODE_INS_PAV_QUT){
          // 🔧 檢查點：Xsens原始數據(根據PROJECT OVERVIEW)
          static unsigned long last_xsens_checkpoint = 0;
          if (millis() - last_xsens_checkpoint > 1000) { // 每秒輸出一次
            Serial.print("🔧 [CHECKPOINT:XSENS:LatLon] ");
            Serial.print(latlon.float_val[0], 6);
            Serial.print(", ");
            Serial.print(latlon.float_val[1], 6);
            Serial.print(", Alt: ");
            Serial.println(hei.float_val, 2);
            
            Serial.print("🔧 [CHECKPOINT:XSENS:Quaternion] ");
            Serial.print(qut.float_val[0], 4);
            Serial.print(", ");
            Serial.print(qut.float_val[1], 4);
            Serial.print(", ");
            Serial.print(qut.float_val[2], 4);
            Serial.print(", ");
            Serial.println(qut.float_val[3], 4);
            last_xsens_checkpoint = millis();
          }
...
void sendMAVLink_GPS_RAW_INT(
  ...
  const my_data_3f &vel, const my_data_3f &ori, const my_data_3f &omg, const my_data_u4 &status){
    ...
    // 🔧 檢查點輸出：GPS_RAW_INT
    outputPacketCheckpoint("GPS_RAW_INT", buffer, len);
...
void sendMAVLink_GPS_INPUT(
  ...
  const my_data_3f &vel, const my_data_3f &ori, const my_data_3f &omg, const my_data_u4 &status){
    ...
    // 🔧 檢查點輸出：GPS_INPUT
    outputPacketCheckpoint("GPS_INPUT", buffer, len);
    Serial1.write(buffer, len); // 🔧 直接使用 Serial1
}
...
uint64_t getPX4Time(bool is_print) {
  ...
  // 🔧 檢查點輸出：TIMESYNC
  outputPacketCheckpoint("TIMESYNC", buffer, len);
  Serial1.write(buffer, len); // 🔧 直接使用 Serial1
...
void checkSTR_CMD(String command){
  ...
  // 🔧 新增檢查點系統和封包監測相關命令(根據PROJECT OVERVIEW)
  else if (command == "PACKET_ON"){
    packet_monitoring_enabled = true;
    Serial.println("🔧 [CHECKPOINT] Packet monitoring ENABLED - HEX output to IDE");
  }
  else if (command == "PACKET_OFF"){
    packet_monitoring_enabled = false;
    Serial.println("🔧 [CHECKPOINT] Packet monitoring DISABLED");
  }
  else if (command == "IDE_STATUS"){
    Serial.println("🔧 [CHECKPOINT] IDE Connection Status:");
    Serial.print("  IDE Connected: ");
    Serial.println(ide_connected ? "YES" : "NO");
    Serial.print("  Packet Monitoring: ");
    Serial.println(packet_monitoring_enabled ? "ENABLED" : "DISABLED");
    Serial.print("  Serial Available: ");
    Serial.println(Serial ? "YES" : "NO");
  }
  else if (command == "SERIAL_STATUS"){
    // 🔧 實時監控三個Serial端口狀態(根據PROJECT OVERVIEW)
    Serial.println("🔧 [CHECKPOINT] Serial Ports Status:");
    Serial.println("========================");
    Serial.print("Serial (USB/IDE): ");
    Serial.print(Serial ? "CONNECTED" : "DISCONNECTED");
    Serial.print(" - Available: ");
    Serial.println(Serial.available());
    
    Serial.print("Serial1 (Pixhawk): 230400 baud - Available: ");
    Serial.println(PIXHAWK_SERIAL.available());
    
    Serial.print("Serial2 (Xsens): 115200 baud - Available: ");
    Serial.println(Serial_xsens.available());
    
    Serial.print("Enable Input: ");
    Serial.println(enable_input ? "YES" : "NO");
  }
  else if (command == "CHECKPOINT_TEST"){
    // 🔧 測試檢查點系統
    Serial.println("🔧 [CHECKPOINT] Testing checkpoint system...");
    
    uint8_t test_data[] = {0xFE, 0x09, 0x23, 0x01, 0x01, 0x00, 0x00, 0x00};
    outputPacketCheckpoint("TEST_HEARTBEAT", test_data, 8);
    
    uint8_t test_odom[] = {0xFE, 0x3C, 0x2A, 0x01, 0x01};
    outputPacketCheckpoint("TEST_ODOMETRY", test_odom, 5);
    
    Serial.println("🔧 [CHECKPOINT] Test packets output completed");
  }
    ...
    // 🔧 立即發送 HEARTBEAT 測試(增強版，根據PROJECT OVERVIEW)
    Serial.println("🔧 [CHECKPOINT] Sending test HEARTBEAT to Pixhawk...");
    sendMAVLink_Heartbeat();
    delay(100);
    ...
    if (Serial1.available()) {
      ...
      int count = 0;
      while (Serial1.available() && count < 100) {
        response[count] = Serial1.read();
        count++;
      }
      
      Serial.println("🔧 [CHECKPOINT] Pixhawk response received:");
      outputPacketCheckpoint("PIXHAWK:RESPONSE", response, count);
    ...
    // 🔧 測試 Xsens 傳感器連接(增強版，根據PROJECT OVERVIEW)
    Serial.println("🔧 [CHECKPOINT] Testing Xsens MTI-680 connection...");
    ...
    while (Serial2.available()) {
      uint8_t b = Serial2.read();
      // 檢查點：輸出清空的數據
      if (packet_monitoring_enabled) {
        Serial.print("🔧 [CHECKPOINT:XSENS:CLEAR] ");
        if (b < 16) Serial.print("0");
        Serial.print(b, HEX);
        Serial.println();
      }
    ...
    // 發送簡單的測試命令
    Serial.println("🔧 [CHECKPOINT] Sending XBUS test command to Xsens...");
    uint8_t xsens_cmd[] = {0xFA, 0xFF, 0x30, 0x00, 0xD1};  // GoToConfig command
    outputPacketCheckpoint("XSENS:CMD:GOTOCONFIG", xsens_cmd, 5);
    
    for (int i = 0; i < 5; i++) {
      Serial2.write(xsens_cmd[i]);
    }
    ...
    if (Serial2.available()) {
      ...
      Serial.println("🔧 [CHECKPOINT] Xsens response received:");
      outputPacketCheckpoint("XSENS:RESPONSE", response, count);
    ...
    Serial.println("🔧 [CHECKPOINT] Serial2 restored to 115200 baud");
  } 
