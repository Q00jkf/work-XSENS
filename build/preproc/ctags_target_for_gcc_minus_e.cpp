# 1 "C:\\Users\\user\\Documents\\GitHub\\Arduino2Pixhawk\\Arduino2Pixhawk.ino"
# 2 "C:\\Users\\user\\Documents\\GitHub\\Arduino2Pixhawk\\Arduino2Pixhawk.ino" 2
# 3 "C:\\Users\\user\\Documents\\GitHub\\Arduino2Pixhawk\\Arduino2Pixhawk.ino" 2
# 4 "C:\\Users\\user\\Documents\\GitHub\\Arduino2Pixhawk\\Arduino2Pixhawk.ino" 2
# 5 "C:\\Users\\user\\Documents\\GitHub\\Arduino2Pixhawk\\Arduino2Pixhawk.ino" 2
# 6 "C:\\Users\\user\\Documents\\GitHub\\Arduino2Pixhawk\\Arduino2Pixhawk.ino" 2
# 14 "C:\\Users\\user\\Documents\\GitHub\\Arduino2Pixhawk\\Arduino2Pixhawk.ino"
XsensUart xsens(Serial2);
...
void loop() {
  ...
  if (ISR_flag_NMEA){
    if (current_output_mode == OUT_MODE_NMEA){
      printNMEAWithModifiedTimestamp(Serial4, Serial1, false);
    }
    else{
      printNMEAWithModifiedTimestamp(Serial4, Serial3, current_output_mode == OUT_MODE_GNSS);
    }
  ...
  if (SerialUSB.available()){
    if (USB_Setting_mode && (current_output_mode == OUT_MODE_XBUS)){
      checkXBUS_CMD(SerialUSB);
    }

    else{
      // checkSTR_CMD(Serial.readString());
      checkSTR_CMD(readCurrentBytes(SerialUSB));
    }
  }

  if (Serial1.available() && enable_input){
    if (current_output_mode == OUT_MODE_BIN && Serial1.available() >= 0x20) {
      ...
      while (Serial1.available()) {
        ...
        if (mavlink_parse_char(MAVLINK_COMM_0, c, &msg, &status)) {
          ...
          send2Serial(Serial1, "Set Output Package to INS_PAV_QUT...");
          send2Serial(Serial1, "Set MAVLINK ODOMETRY(331) Output...");
          ...
          setXsensPackage();
          sendProcessingDataAndStartMeas(Serial1);
          break;
        }
      }
    }
      ...
      uint8_t buffer[200];
      uint16_t idx = 0;
      while (Serial1.available() && idx < 200) {
      ...
      if (idx > 0) {
        if (idx >= 6){
          if (buffer[0] == 'C' && buffer[1] == 'O' && buffer[2] == 'N' && buffer[3] == 'F' && buffer[4] == 'I' && buffer[5] == 'G'){
            ...
            send2Serial(Serial1, "Enter CONFIG mode");
          }
        }
        if (current_output_mode == OUT_MODE_XBUS) { Serial2.write(buffer, idx); }
      }
    }

    else{
      // checkSTR_CMD(PIXHAWK_SERIAL.readString()); 
      checkSTR_CMD(readCurrentBytes(Serial1));
    }
  }
}
...
void readXsens(){
  if (Serial2.available()){
    if (current_output_mode == OUT_MODE_XBUS){
      uint8_t buffer[200];
      uint16_t idx = 0;
      while (Serial2.available() && idx < 200) {
        buffer[idx++] = Serial2.read();
      }
      if (idx > 0) {
        if (USB_Setting_mode) { SerialUSB.write(buffer, idx); }
        else { Serial1.write(buffer, idx); }
      }
    }
      ...
      if (xsens.getDataStatus() == DATA_OK) {
        ...
        if (current_output_mode == OUT_MODE_ML_ODOM && current_Xsens_mode == MODE_INS_PAV_QUT){
          ...
          if (current_Xsens_mode == MODE_INS_PAV_QUT){
            ...
            xsens.getFilterStatus(status, true, Serial1);
          ...
          else { SerialUSB.println("String output doesn't supportan this mode yet!"); }
        }
          ...
          if (current_Xsens_mode == MODE_AHRS){
            ...
            Serial1.write(buffer, 76);
          }
          else {
            Serial1.println("Binary output doesn't supported this mode yet!");
          }
        }
    ...
    if (Serial1.available() > 0) {
      ...
      if (mavlink_parse_char(MAVLINK_COMM_0, c, &msg_recv, &status)) {
        if (msg_recv.msgid == 111) {
          ...
          if (is_print) {
            SerialUSB.print("ts1: ");
            SerialUSB.println(ts_recv.ts1 * 0.001, 3); // 微秒轉毫秒
            SerialUSB.print("tc1: ");
            SerialUSB.println(ts_recv.tc1 * 1e-9, 3); // 微秒轉毫秒
          }
          return ts_recv.tc1;
        }
      }
...
void setXsensPackage(){
  send2Serial(Serial1, "MODE: " + String(current_Xsens_mode));
  xsens.ToConfigMode();
  // xsens.getFW();
  xsens.reqPortConfig();
  ...
  if (current_output_mode == OUT_MODE_ML_ODOM) {
    ...
    else { send2Serial(Serial1, "This mode is not supported yet!"); }
  }
...
void checkPX4CON(){
  ...
  if (counter == 5){ SerialUSB.println("PX4CON not connected!"); }
  else { SerialUSB.println("PX4CON connected!"); }
}

bool checkUSBSerial(){
  for (int i=0;i<5;i++){
    if (!SerialUSB) { delay(10); }
    else {
      delay(3000);
      SerialUSB.println("Connect to PC");
      return true;
    }
  }
  return false;
...
void checkSTR_CMD(String command){
  command.trim();
  send2Serial(Serial1, "Command: " + command);
  if (command == "AHRS"){
    current_Xsens_mode = MODE_AHRS;
    send2Serial(Serial1, "Set Output Package to AHRS...");
    setXsensPackage();
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "AHRS_QUT"){
    current_Xsens_mode = MODE_AHRS_QUT;
    send2Serial(Serial1, "Set Output Package to AHRS_QUT...");
    setXsensPackage();
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "INS"){
    current_Xsens_mode = MODE_INS;
    send2Serial(Serial1, "Set Output Package to INS...");
    setXsensPackage();
    sendProcessingDataAndStartMeas(Serial1);
  }
    ...
    send2Serial(Serial1, "Set Output Package to IMU...");
    setXsensPackage();
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "GNSS_RAW"){
    current_output_mode = MODE_GNSS_RAW;
    send2Serial(Serial1, "Set Output Package to GNSS_RAW...");
    setXsensPackage();
    sendProcessingDataAndStartMeas(Serial1);
  }
    ...
    current_output_mode = OUT_MODE_ML_GPS_RAW;
    send2Serial(Serial1, "Set MAVLINK GPS_INPUT(232) Output...");
    setXsensPackage();
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "ML_VISO"){
    current_output_mode = OUT_MODE_ML_VISO;
    send2Serial(Serial1, "Set MAVLINK VISION_POSITION_ESTIMATE(102) Output...");
    setXsensPackage();
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "VEC"){
    Serial1.println("$VNRRG,01,VN-300*58");
    SerialUSB.println("Send: $VNRRG,01,VN-300*58");
    Serial1.println("$VNRRG,01,VN-310*58");
    SerialUSB.println("Send: $VNRRG,01,VN-310*58");
  ...
  // else if (command == "NMEA"){
    ...
    send2Serial(Serial1, "Set GNSS Test Mode ...");
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "USB"){
    USB_Setting_mode = true;
    current_output_mode = OUT_MODE_CONFIG;
    send2Serial(Serial1, "Set USB Configuration Mode ...");
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "USB_OUT"){
    send2Serial(Serial1, "Leave USB Configuration Mode ...");
    USB_Setting_mode = false;
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "CONFIG") {
    current_output_mode = OUT_MODE_CONFIG;
    send2Serial(Serial1, "Set CONFIG Mode...");
    sendProcessingDataAndStartMeas(Serial1);
  }
  else if (command == "RESET") {
    send2Serial(Serial1, "Reseting...");
    xsens.ToConfigMode();
    xsens.reset();
    xsens.InitMT();
    sendProcessingDataAndStartMeas(Serial1);
    ...
    xsens.caliGyro(10, &Serial1);
  }

  else if (command == "INIT_XSENS" && USB_Setting_mode){
    SerialUSB.println("Start to config Xsens MTI-680");
    MyQuaternion::Quaternion qut(0, ((180)*0.017453292519943295769236907684886), ((-90)*0.017453292519943295769236907684886));
    xsens.ToConfigMode();
    ...
    SerialUSB.println("Start to config LOCOSYS");
    for (int i=0;i<5;i++){
      send2Serial(Serial4, "$PAIR003*39");
      if (checkLOCOSYS_ACK(Serial4)) { break; }
    }

    send2Serial(Serial4, "$PAIR062,2,1*3D"); // enable GSA
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR513*3D");
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR002*38");
    checkLOCOSYS_ACK(Serial4);
    delay(1000);
    for (int i=0;i<5;i++){
      send2Serial(Serial4, "$PAIR003*39");
    ...
    send2Serial(Serial4, "$PAIR062,4,1*3B"); // enable RMC
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR513*3D");
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR002*38");
    ...
    send2Serial(Serial4, "$PAIR062,0,1*3F"); // enable GGA
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR513*3D");
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR002*38");
    ...
    send2Serial(Serial4, "$PAIR062,8,1*37"); // enable GST
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR513*3D");
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR002*38");
    ...
    send2Serial(Serial4, "$PAIR062,3,0*3D"); // disable GSV
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR513*3D");
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR002*38");
    ...
    send2Serial(Serial4, "$PAIR062,5,0*3B"); // disable VTG
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR513*3D");
    checkLOCOSYS_ACK(Serial4);
    send2Serial(Serial4, "$PAIR002*38");
